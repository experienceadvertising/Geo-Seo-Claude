...
export function DashboardWalkthrough({ auditId, paid }: { auditId?: number; paid: boolean }) {
  ...
  const steps = [
    ...
    { title: "Choose one practical improvement", body: "Open Top actions in the navigation. Start with one recommendation, read the evidence and instructions, and make the change on your website. Mark it complete only after you have implemented it.", href: auditId ? `/results/${auditId}#recommendations` : undefined, action: "Open my top actions" },
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-white p-4" aria-label="Dashboard help">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
          ...
          {current.href ? <Link href={current.href} onClick={() => { setOpen(false); if (step === 0) requestAnimationFrame(() => document.getElementById("baseline-url")?.focus()); }}><Button className="w-full">{current.action}</Button></Link> : <p className="text-sm text-muted-foreground">Run your first audit to unlock this step.</p>}
