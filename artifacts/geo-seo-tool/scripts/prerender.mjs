...
function buildStaticContent(route) {
  ...
  const sections = (route.staticSections || []).map(({ heading: sectionHeading, body }) => (
    `<section style="margin-top:36px"><h2>${escapeHtmlText(sectionHeading)}</h2><p>${escapeHtmlText(body)}</p></section>`
  )).join("\n    ");
  ...
  // Every route's sections/faqs come from the manifest (STATIC_CONTENT map).
  // Nothing generic is injected here beyond a short related-pages nav —
  // shared boilerplate across routes previously made the static bodies ~75%
  // identical, which is exactly the duplicate-content pattern this product
  // audits other sites for.
