...
const baseUrl = process.env.QA_BASE_URL || 'http://localhost:4199';
const browser = await chromium.launch({ executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', headless: true });
try {
  const context = await browser.newContext();
  await context.route('**/*', route => ['localhost', '127.0.0.1'].includes(new URL(route.request().url()).hostname) ? route.continue() : route.abort());
  const page = await context.newPage();
  await page.goto(`${baseUrl}/actions/1?task=direct-answer-block#recommendations`);
  const focusedRewrite = page.getByRole('region', { name: 'Selected improvement' }).getByRole('region', { name: 'Suggested site-specific rewrite' });
  await focusedRewrite.waitFor();
  assert.equal(await focusedRewrite.getByText('We help growth teams find and fix gaps in their analytics.').count(), 1);
  ...
  const records = await (await context.request.get(`${baseUrl}/api/geo/recommendation-progress?domain=example.com`)).json();
  ...
  const reopened = await (await context.request.get(`${baseUrl}/api/geo/recommendation-progress?domain=example.com`)).json();
  ...
  const onsite = await (await context.request.get(`${baseUrl}/api/geo/recommendation-progress?domain=example.com`)).json();
